      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[94vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-3 gap-6">
            {/* Left column: revision inputs */}
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">修改建议</label>
                <div className="flex flex-wrap gap-2 mb-3">
                  {SUGGESTION_CHIPS.map((chip) => (
                    <button key={chip} onClick={() => setSuggestion(chip)} className={`suggestion-chip ${suggestion === chip ? 'active' : ''}`}>{chip}</button>
                  ))}
                </div>
                <textarea
                  value={suggestion}
                  ...
                  rows={3}
                  placeholder="输入本次修订的自由建议"
                  className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-gray-900 outline-none resize-none text-sm"
                />
              </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="text-sm text-gray-700 whitespace-pre-wrap editor-font bg-white border border-gray-200 rounded-xl p-4 min-h-[300px] overflow-y-auto">
                {selectedText || '请先在正文中选中要修订的内容'}
              </div>
            </div>

            {/* Right column: candidate + paragraph review */}
            <div className="space-y-4">
              <div>
                <div className="text-xs font-semibold text-gray-400 uppercase mb-2">修订候选稿</div>
                <textarea
                  value={candidateText}
                  onChange={(e) => setCandidateText(e.target.value)}
                  rows={10}
                  placeholder={isGenerating ? 'AI 正在生成中...' : '点击生成后出现修订内容'}
                  className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-gray-900 outline-none resize-none text-sm"
              ...
              {/* Paragraph review */}
              {paragraphChoices.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700">逐段审阅</span>
                    <div className="flex gap-2">
                      <button onClick={handleAcceptAllOriginal} className="px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded transition-colors">
                        全部保留原文
                      </button>
                      <button onClick={handleAcceptAllCandidate} className="px-3 py-1 text-xs font-medium text-emerald-600 hover:bg-emerald-50 rounded transition-colors">
                        全部采纳候选
                      </button>
                    </div>
                  </div>
                          ...
                          <div className="p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="bg-gray-50 rounded-xl border border-gray-200 p-3">
                                <div className="text-xs font-semibold text-gray-400 uppercase mb-2">原文差异</div>
                              ...
                              <div className="bg-emerald-50 rounded-xl border border-emerald-200 p-3">
                                <div className="text-xs font-semibold text-emerald-700 uppercase mb-2">候选稿差异</div>
        ...
        <div className="p-6 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3">
          <button
            ...
            className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-white disabled:opacity-50 transition-colors"
          >
